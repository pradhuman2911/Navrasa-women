...
export default function AboutSection() {
  ...
  return (
    <section
      id="about"
      className="min-h-screen flex flex-col items-center justify-center px-6 md:px-20 py-20 bg-gradient-to-b from-[#2a1e56] to-[#1f1644] text-white relative overflow-hidden"
    >
      {/* Decorative elements */}
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0">
        <div className="absolute top-10 left-10 w-64 h-64 bg-purple-600 rounded-full filter blur-[100px] opacity-20"></div>
        <div className="absolute bottom-10 right-10 w-80 h-80 bg-indigo-500 rounded-full filter blur-[120px] opacity-20"></div>
        <motion.div 
          className="absolute top-1/4 right-1/4 w-6 h-6 bg-purple-300 rounded-full"
          animate={{ 
            y: [0, -15, 0], 
            opacity: [0.8, 1, 0.8]
          }}
      ...
      <div className="max-w-6xl mx-auto z-10 space-y-16">
        ...
        >
          ...
          >
            <span className="px-4 py-1.5 bg-purple-500/20 text-purple-300 text-sm font-medium rounded-full">
              About Navrasa
            </span>
          </motion.div>
          
          <h2 className="text-3xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-300 to-indigo-300">
        ...
        {/* Stats section */}
        <motion.div 
          className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-0"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
        ...
        >
          {stats.map((stat, index) => (
            ...
            >
              <span className="text-4xl md:text-5xl font-bold text-purple-300 mb-2">
                {stat.value}
              </span>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[#231a48]/50 backdrop-blur-sm rounded-2xl p-8 border border-purple-500/10">
            {activeTab === "mission" && (
              <motion.div
        ...
        >
          <motion.h3 
            variants={itemVariants}
            className="text-2xl md:text-3xl font-bold text-center text-purple-300"
          >
            Our Three Pillars
          </motion.h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {pillars.map((pillar, index) => (
              <motion.div
                key={index}
                variants={itemVariants}
                className="bg-[#231a48]/40 backdrop-blur-sm rounded-xl p-6 border border-purple-500/10 hover:border-purple-500/30 transition-all hover:shadow-lg hover:shadow-purple-500/10 flex flex-col items-center text-center"
                whileHover={{ y: -5, transition: { duration: 0.2 } }}
              >
                <span className="text-4xl mb-4">{pillar.icon}</span>
                <h4 className="text-xl font-semibold text-purple-300 mb-3">{pillar.title}</h4>
                <p className="text-purple-100/90">{pillar.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        >
          <a 
            href="#join" 
            className="inline-block px-8 py-3 bg-gradient-to-r from-purple-500 to-indigo-600 text-white font-medium rounded-full hover:shadow-lg hover:shadow-purple-500/25 transition-all hover:-translate-y-1"
          >
            Join Our Community
          </a>
        </motion.div>
